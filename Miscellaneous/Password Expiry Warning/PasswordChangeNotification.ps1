...
foreach ($user in $users) {
	...
	if (((Get-Date) - $user.PasswordLastSet).Days -gt $maxPasswordAge) {
		...
		Write-Warning "The password for $samAccountName was last changed $(((Get-Date) - $user.PasswordLastSet).Days) days ago"
	}
...
if ($testing) {
	Write-Output "Testing mode enabled, ignoring user addresses"
}

<#
if ($testing -and $testRecipient) {
	Write-Output "Testing mode enabled, sending one message to the test address"
	
    $subject="Your password will expire 0 days"
    # Email Body Set Here, Note You can use HTML, including Images.
    $body ="
    <font face=""verdana"">
    Dear $testRecipient,
    <p>Your Password will expire $messageDays.</p>
    <p>To change your password on a PC press CTRL ALT Delete and choose Change Password. Alternatively, visit our <a href=""https://passwordreset.whsc.vic.edu.au"">Password Reset portal</a> to update your password. Please take the opportunity to set your WHSC Secret Questions & Answers if you haven't already done so; these provide the ability to reset your password if you ever forget it.</p>
    <p>Don't forget to update your password on any mobile devices you've configured (for example, the Outlook app on your phone). </p>
    <p>Thanks, <br>WHSC IT Department</p>
    <p><a href=""mailto:it@blah.com?Subject=Password Expiry Assistance"">it@blah.com</a></p>
    </font>"
    ...
    try {
        Send-Mailmessage -smtpServer $smtpServer -from $from -to $testRecipient -subject $subject -body $body -bodyasHTML -priority High -Encoding $textEncoding -ErrorAction Stop
...
foreach ($user in $notifyUsers) {
    ...
    Dear $name,
    <p>Your Password will expire $messageDays</p>
    <p>To change your password on a PC press CTRL ALT Delete and choose Change Password. Alternatively, visit our <a href=""https://passwordreset.whsc.vic.edu.au"">Password Reset portal</a> to update your password.</p>
    <p>Don't forget to update your password on any mobile devices you've configured (for example, the Outlook app on your phone). </p>
    <p>Thanks, <br>WHSC IT Department</p>
    <p><a href=""mailto:it@blah.com?Subject=Password Expiry Assistance"">it@blah.com</a></p>
    </font>"
	
	$body = (Get-Content "C:\Scripts\Password Expiry Warning\PasswordChangeNotification.html").Replace("###messageDays###", $messageDays) | Out-String
    ...
    try {
		...
		write-output "send email for $emailAddress disabled temporarily"
        $user | Add-Member -MemberType NoteProperty -Name SendMail -Value "OK"
    } catch {
        $errorMessage = $_.exception.Message
